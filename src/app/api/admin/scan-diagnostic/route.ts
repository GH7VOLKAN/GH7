/**
 * /api/admin/scan-diagnostic
 *
 * AI sorgu ve scan probleminin kök nedenini anlamak için.
 * Admin-only.
 *
 * Döndürür:
 *  - ENV VAR durumu (hangi API key'ler set, hangileri missing)
 *  - Her AI provider için canlı ping testi
 *  - Kullanıcının brand'inin son 10 scan'i (status, duration, failed count)
 *  - Prompt/PromptResult sayıları
 *  - Son FailedResult kayıtları
 *  - Sonar (Perplexity) quick test
 */

import { NextResponse } from "next/server";
import { prisma } from "@/lib/db";
import { createClient } from "@/lib/supabase/server";
import { isAdmin } from "@/lib/admin";
import { getAllProviderInstances } from "@/lib/ai/provider-registry";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";
export const maxDuration = 60;

interface EnvCheck {
  name: string;
  present: boolean;
  length: number;
}

interface ProviderCheck {
  platform: string;
  available: boolean;
  pingOk: boolean;
  pingMs: number | null;
  pingError: string | null;
}

export async function GET() {
  try {
    // Admin check
    const supabase = await createClient();
    const {
      data: { user },
    } = await supabase.auth.getUser();
    if (!user) {
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }
    if (!isAdmin({ email: user.email, phone: user.phone ?? null })) {
      return NextResponse.json({ error: "Admin only" }, { status: 403 });
    }

    // 1. ENV VAR check
    const envVars: EnvCheck[] = [
      "OPENAI_API_KEY",
      "GH7_ANTHROPIC_API_KEY",
      "ANTHROPIC_API_KEY",
      "GOOGLE_AI_API_KEY",
      "PERPLEXITY_API_KEY",
      "SERPAPI_KEY",
      "DATAFORSEO_LOGIN",
      "DATAFORSEO_PASSWORD",
      "DATABASE_URL",
      "DIRECT_URL",
      "UPSTASH_REDIS_REST_URL",
      "UPSTASH_REDIS_REST_TOKEN",
      "NEXT_PUBLIC_SUPABASE_URL",
      "SUPABASE_SERVICE_ROLE_KEY",
      "RESEND_API_KEY",
    ].map((name) => {
      const v = process.env[name];
      return {
        name,
        present: !!v && v.length > 0,
        length: v?.length ?? 0,
      };
    });

    // 2. Provider availability + quick ping
    const providers = getAllProviderInstances();
    const providerChecks: ProviderCheck[] = await Promise.all(
      providers.map(async (p) => {
        const available = p.isAvailable();
        if (!available) {
          return {
            platform: p.platform,
            available: false,
            pingOk: false,
            pingMs: null,
            pingError: "API key yok",
          };
        }
        const start = Date.now();
        try {
          const result = await Promise.race([
            p.sendPrompt("Türkiye'nin başkenti nedir? Tek kelime cevap ver."),
            new Promise<never>((_, reject) =>
              setTimeout(() => reject(new Error("Timeout 30s")), 30000),
            ),
          ]);
          const ms = Date.now() - start;
          if (result.error) {
            return {
              platform: p.platform,
              available: true,
              pingOk: false,
              pingMs: ms,
              pingError: result.error.slice(0, 200),
            };
          }
          return {
            platform: p.platform,
            available: true,
            pingOk: !!result.content && result.content.length > 0,
            pingMs: ms,
            pingError: null,
          };
        } catch (err) {
          return {
            platform: p.platform,
            available: true,
            pingOk: false,
            pingMs: Date.now() - start,
            pingError: err instanceof Error ? err.message : String(err),
          };
        }
      }),
    );

    // 3. Kullanıcının brand'leri + son 10 scan
    const profile = await prisma.profile.findUnique({
      where: { id: user.id },
    });
    const brands = profile
      ? await prisma.brand.findMany({
          where: { profileId: profile.id },
          select: { id: true, name: true, domain: true },
          take: 10,
        })
      : [];

    const brandIds = brands.map((b) => b.id);

    const recentScans =
      brandIds.length > 0
        ? await prisma.scan.findMany({
            where: { brandId: { in: brandIds } },
            orderBy: { startedAt: "desc" },
            take: 10,
            include: {
              _count: { select: { results: true } },
            },
          })
        : [];

    const promptCount =
      brandIds.length > 0
        ? await prisma.prompt.count({
            where: { brandId: { in: brandIds } },
          })
        : 0;

    const promptResultCount =
      brandIds.length > 0
        ? await prisma.promptResult.count({
            where: { scan: { brandId: { in: brandIds } } },
          })
        : 0;

    // 4. Son failed results (FailedResult tablosu varsa)
    let failedResults: unknown[] = [];
    try {
      failedResults = await (
        prisma as unknown as {
          failedResult: {
            findMany: (args: unknown) => Promise<unknown[]>;
          };
        }
      ).failedResult.findMany({
        where: { promptId: { in: [] } }, // prompt ids eklemek için sonra
        take: 10,
        orderBy: { createdAt: "desc" },
      });
    } catch {
      failedResults = [];
    }

    // 5. Teşhis özeti — en olası sorun
    const availableCount = providerChecks.filter((p) => p.available).length;
    const pingingCount = providerChecks.filter((p) => p.pingOk).length;
    let likelyIssue: string;
    if (availableCount === 0) {
      likelyIssue =
        "🔴 KRİTİK: Hiçbir AI provider aktif değil. Vercel env vars'da API key'ler eksik veya yanlış.";
    } else if (pingingCount === 0) {
      likelyIssue =
        "🔴 KRİTİK: Hiçbir provider ping'e cevap vermedi. Key'ler var ama geçersiz ya da API'ler kapalı olabilir.";
    } else if (pingingCount < availableCount) {
      likelyIssue = `🟡 KISMEN: ${pingingCount}/${availableCount} provider çalışıyor. Geri kalanların key'leri geçersiz veya quota bitmiş.`;
    } else if (promptCount === 0) {
      likelyIssue =
        "🟡 Hiç Prompt yok. /analiz yapıldı mı? Discovery aşamasında hata olmuş olabilir.";
    } else if (promptResultCount === 0) {
      likelyIssue =
        "🟡 Prompt var ama PromptResult boş. Scan tetiklenmemiş veya tetiklendi ama executeScan silent fail ediyor. Vercel function logs kontrol edin.";
    } else {
      likelyIssue = "✅ Veri akışı sağlıklı görünüyor.";
    }

    return NextResponse.json({
      summary: {
        envVarsOk: envVars.filter((e) => e.present).length,
        envVarsTotal: envVars.length,
        providersAvailable: availableCount,
        providersPinging: pingingCount,
        promptCount,
        promptResultCount,
        brandCount: brands.length,
        recentScanCount: recentScans.length,
        likelyIssue,
      },
      envVars,
      providers: providerChecks,
      brands,
      recentScans: recentScans.map((s) => ({
        id: s.id,
        brandId: s.brandId,
        status: s.status,
        type: s.type,
        startedAt: s.startedAt,
        completedAt: s.completedAt,
        resultCount: s._count.results,
        durationMs:
          s.completedAt && s.startedAt
            ? s.completedAt.getTime() - s.startedAt.getTime()
            : null,
      })),
      counts: {
        prompts: promptCount,
        promptResults: promptResultCount,
        failedResults: failedResults.length,
      },
      timestamp: new Date().toISOString(),
    });
  } catch (err) {
    console.error("[scan-diagnostic] Error:", err);
    return NextResponse.json(
      {
        error: "Diagnostic failed",
        message: err instanceof Error ? err.message : String(err),
      },
      { status: 500 },
    );
  }
}
